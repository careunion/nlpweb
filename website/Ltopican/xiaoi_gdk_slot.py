...
def Slotdistancefind(queryobj):
    ...
    if not re.search(u'附近|旁边|周围|这里|公里|里|千米|k米|米|分钟|小时|分|步行|走路|开车|坐车|打车|乘车|打的|距离|路程|远近',query):
        return None
    ...
    if _obj:
        ...
        for distance_cn in _obj:
            ...
            #if type_cn == u'以内' or type_cn == u'之内' or type_cn == u'以下' or distance_cn[2]:
            distance = u'0米,' + distance
    pattern4 = re.compile(u'(' + vaguestr + '?' + markstr + type1str + '.{0,3}?(' + numextend + ')' + '[^公里|里|千米|k米|米|分|分钟|小时]*' + type2str + u')')
...
def Slotpricefind(queryobj, maxPrice = 1000000):
    ...
    if not re.search(u'元钱|块钱|元|块|价格|价钱|单价|消费|每人|人均|价位|价码|金额',query):
        return None
...
def Slotattentionfind(queryobj, maxAttention = 1000):
    ...
    if not re.search(u'关注|关注度|关注量|访问量|购买量|人流量|点击量',query):
        return None
